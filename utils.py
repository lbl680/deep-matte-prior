...
def plot_image_grid(name, image_np, output_path="output/"):
    """
    ...
    # if image_np[0].shape[0] == 1:
    #     plt.imshow(image_np, cmap='gray', interpolation=interpolation)
    # else:
    #     plt.imshow(image_np.transpose(1, 2, 0), interpolation=interpolation)
    ...
    cv2.imwrite(os.path.join(output_path, "{}.png".format(name)), image_np)
...
def prepare_image(file_name):
    ...
    img, img_np = get_image(file_name, -1)

    return img_np  # pil_to_np(img_pil)
